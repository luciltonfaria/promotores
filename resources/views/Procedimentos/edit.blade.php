<!-- resources/views/procedimentos/edit.blade.php -->
<!DOCTYPE html>
<html lang="en">
...
<body>
                                ...
                                <div class="col-sm-12">
                                    <div class="card">
                                        <div class="card-header d-flex align-items-center justify-content-between">
                                            <h5 class="card-title">Formulário de Procedimento</h5>
                                            <a href="{{ url('/procedimentos') }}" class="btn btn-primary ms-auto">Voltar</a>
                                        </div>
                                        <div class="card-body">
                                            
                                            <!-- Form starts -->
                                            <form action="{{ url('/procedimentos/'.$procedimento->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="mb-3 col-3">
                                                    <label for="codigo" class="form-label">Código</label>
                                                    <input type="text" class="form-control" id="codigo" name="codigo" maxlength='6' value="{{ $procedimento->codigo }}" required>
                                                </div>
                                                <div class="mb-3">
                                                ...
                                                <div class="mb-3 col-3">
                                                    <label for="valor_base" class="form-label">Valor</label>
                                                    <input type="text" class="form-control" id="valor_base" name="valor_base" value="{{ number_format($procedimento->valor_base, 2, ',', '.') }}" required>
                                                </div>
    ...
    <script>
        $(document).ready(function(){
            // Aplica a máscara de valor com o formato ###.###,##
            $('#valor_base').mask('000.000,00', {reverse: true});

            // Ao submeter o formulário, remove a máscara e ajusta o valor para o formato correto
            $('form').on('submit', function() {
                var valor = $('#valor_base').val();
                // Remover pontos e substituir a vírgula por ponto
                valor = valor.replace(/\./g, '').replace(',', '.');
                $('#valor_base').val(valor);
            });
        });
    </script>
